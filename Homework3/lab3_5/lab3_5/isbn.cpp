...
Isbn::Isbn(string tara, string editura, string titlu) {
	...
	if(num == 10) {
		...
		cifra_control = to_string(num)[0];//imi trebuie un caracter
	}
	nr_i++;
	cout << "s-a apelat constructorul general cu 3 parametrii";
} //344-233-123-2
Isbn::Isbn(string isbn) {
	...
	nr_i++;
	cout << "s-a apelat constructorul general cu un param";
}
